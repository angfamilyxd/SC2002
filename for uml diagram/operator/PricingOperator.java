...
public class PricingOperator {
     ...
     */
    private static double applyCinemaClassFactor(CinemaClassType cinemaClass){
    ...
    private static double applyDayOfWeekOrPHFactor(double price, LocalDate date){
        ...
        for (PublicHoliday ph:PriceCalculator.getInstance().getPublicHolidays()){
            if (ph.getDate().isEqual(date)){
                return price + PriceCalculator.getInstance().getPublicHolidayIncrement();
            }
        }

        //if not in public holiday, check if is in weekend
        if ((date.getDayOfWeek() == DayOfWeek.SATURDAY) || (date.getDayOfWeek() == DayOfWeek.SUNDAY)){
            return price + PriceCalculator.getInstance().getWeekendIncrement();
        }
    ...
    public static double applyGSTFactor(double price){
        return (price + price * PriceCalculator.getInstance().getGstPercentageIncrease());
    }
}