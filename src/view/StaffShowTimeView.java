...
public class StaffShowTimeView extends StartView {
     ...
     */
    public StaffShowTimeView(Movies selectedMovie){
    ...
    private void handleGettingInfo(){
        cineplexes = DataOperator.retrieveAllCineplexes();
        ArrayList<String> cineplexesString = new ArrayList<>();
        for (Cineplex cineplex : cineplexes){
            cineplexesString.add(cineplex.getName());
        }
        FileInputOutputOperator.printMenuOptions(cineplexesString);
        chosenCineplexIndex = FileInputOutputOperator.getUserInputInt("Please choose one of the cineplexes",1,cineplexes.size());
        ...
        FileInputOutputOperator.printMenuOptions(cinemaStrings);
        chosenCinemaIndex = FileInputOutputOperator.getUserInputInt("Please choose one of the cinemas",1,cinemaStrings.size());
        chosenDate = FileInputOutputOperator.getUserInputDate("Please input the date that you want to insert the showtime to: ");
    }
    ...
    private void handleRemoveShowTime(){
        ...
        int removeChoice = FileInputOutputOperator.getUserInputInt("Please input the number of the showtime that you want to remove: ",1,maxlength);
        ...
        DataOperator.overwriteCineplexDatabaseWithNewCineplexes(cineplexes);
    ...
    private void handleAddShowTime(){
        ...
        int hourComponent = FileInputOutputOperator.getUserInputInt("",0,23);
        System.out.println("Please input the minute component (24-hour format), range (0-59)");
        int minuteComponent = FileInputOutputOperator.getUserInputInt("",0,59);
        ...
        FileInputOutputOperator.printMenuOptions(movieTypeListStrings);
        int typeChoice = FileInputOutputOperator.getUserInputInt("Input the choice: ",1,movieTypeListStrings.size());
        ...
        int userSaveChoice = FileInputOutputOperator.getUserInputInt("1 - Proceed to save, 0 - Cancel",0,1);
